...
export default class Connection {
  ...
  setOption(name: string, value: any) {
    ...
    localStorage.setItem("rustdesk-web-peers", JSON.stringify(peers));
  }
...
function getrUriFromRs(
  ...
  const explicitScheme = uri.match(/^(wss?):\/\//i)?.[1]?.toLowerCase();
  const configuredScheme = localStorage.getItem("rustdesk-websocket-scheme");
  const scheme = explicitScheme ||
    (configuredScheme && configuredScheme !== "auto" ? configuredScheme : null) ||
    (window.location.protocol === "https:" ? "wss" : "ws");
  const authority = uri.replace(/^wss?:\/\//i, "").split("/")[0];
  const match = authority.match(/^(.+?)(?::(\d+))?$/);
  const host = match?.[1] || authority;
  const basePort = match?.[2] ? Number.parseInt(match[2], 10) : PORT;
  const targetPort = basePort + (isRelay ? roffset || 3 : 2);
